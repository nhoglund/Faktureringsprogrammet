...
namespace Fakturering
{
    ...
    { 
        ...
        public void DrawRoundRectangle(System.Drawing.Pen pen, RectangleF rectangle, float radius)
		{
            GraphicsPath path = this.GetRoundedRect(rectangle, radius); 
		...
		public void DrawDoubleLine(double x1, double x2, double y, double height, Pen pen)
		{
			...
			for(int i=(int)y; i<y+height; i++) {
				if(i<y+thinheight || i >= y+skipheight) {
					mGraphics.DrawLine(pen, new Point((int)x1, i), new Point((int)x2, i));
				}
			}
		}
